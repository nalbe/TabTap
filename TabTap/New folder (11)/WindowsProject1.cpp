...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static PROCESS_INFORMATION pi; // Store process information
	...
	case WM_LBUTTONUP:
	{
		...
		if (hWndOSK) {
			...
			WriteRegistry(OSKKey, OSKSubKey, L"WindowTop", windowRect.top - (WindowHeightOSK - sizeCollapsed.cy) / 2);
			// Run OSK
			//if (!CreateProcess(OSKPathEX, NULL, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &si, &pi)) {
			if (!CreateProcess(OSKPathEX, NULL, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
				MessageBox(NULL, L"Failed to start osk.exe!", L"Error", MB_OK | MB_ICONERROR);
			}
			while (!(hWndOSK = FindWindow(OSKClassName, NULL))) {
				Sleep(50);  // Sleep briefly to reduce CPU usage.
			}
